...
from .utils import Binary_classification_metric
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            if (step + 1) % gradient_accumulation_steps == 0:
                ...
                if global_step % print_step == 0 and global_step != 0:
                    ...
                    train_preci, train_recall, train_F1, train_acc, train_auc = Binary_classification_metric(all_preds, all_labels, label_list)
                    dev_loss, dev_preci, dev_recall, dev_F1, dev_acc, dev_auc = evaluate(model, dev_dataloader, criterion, device, label_list)
                    ...
                    writer.add_scalar("precision/train", train_preci, c)
                    writer.add_scalar("precision/dev", dev_preci, c)

                    writer.add_scalar("recall/train", train_recall, c)
                    writer.add_scalar("recall/dev", dev_recall, c)
...
def evaluate(model, dataloader, criterion, device, label_list):
    ...
    preci, recall, F1, acc, auc = Binary_classification_metric(all_preds, all_labels, label_list)
    return epoch_loss/len(dataloader), preci, recall, F1, acc, auc


def eval_save(model, dataloader, criterion, device, label_list):
    ...
    with open("preds.csv", 'w', encoding='utf-8') as f:
        ...
        for i in range(len(all_preds)):
            out_writer.writerow([all_labels[i], all_preds[i]])
